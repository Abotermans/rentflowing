...
ins("lease_amendment_changes",
  ...
  initialAmendmentChanges.map(x => [
    ...
    x.oldValue === null || x.oldValue === undefined ? "NULL" : `'${JSON.stringify(x.oldValue).replace(/'/g, "''")}'::jsonb`,
    x.newValue === null || x.newValue === undefined ? "NULL" : `'${JSON.stringify(x.newValue).replace(/'/g, "''")}'::jsonb`,
